...
def cleanup_temp_files(base: str):
    for f in glob.glob(f"{base}.*"):
        try:
            ...
            config.logger.warning(f"Failed to remove temp file {f}: {e}")
...
async def get_dislikes(video_id: str) -> Optional[int]:
    ...
    try:
        ...
        config.logger.warning(f"Failed to fetch dislikes for {video_id}: {e}")
        return None
...
async def search_multiple(query: str) -> List[Dict[str, Any]]:
    ydl_opts = {
        ...
        'executable': config.YDL_EXECUTABLE_PATH, 
    }
    def search():
...
async def download_by_url(url: str):
    ...
    def pre_check_and_download():
        ...
        with YoutubeDL(info_opts) as ydl:
            ...
            if info.get("duration") and info['duration'] > config.MAX_SONG_DURATION_SEC:
                raise Exception("LONG_AUDIO")

            filesize_estimate = info.get('filesize') or info.get('filesize_approx')

            if filesize_estimate is not None and filesize_estimate > config.MAX_FILE_SIZE_BYTES:
        ...
        download_opts = {
            ...
            'noplaylist': True,
            'quiet': True,
            'outtmpl': os.path.join(config.TEMP_PATH, f'{unique_id}.%(ext)s'),
            'writethumbnail': True,
            'extractor_args': {'youtube': {'client': 'android'}},
            'no_warnings': True,
            'executable': config.YDL_EXECUTABLE_PATH,
        ...
        with YoutubeDL(download_opts) as ydl:
            ...
            if audio_file and os.path.getsize(audio_file) > config.MAX_FILE_SIZE_BYTES:
